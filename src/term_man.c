...
int main() {
    ...
    map = new_map(alloc(Map));
    map->get_map_level(map);

    pacman = new_pacman(alloc(Pacman));

    blinky = new_ghost_blinky(alloc(Ghost));
    pinky = new_ghost_pinky(alloc(Ghost));
    inky = new_ghost_inky(alloc(Ghost));
    clyde = new_ghost_clyde(alloc(Ghost));
...
void draw_pacman(unsigned char input) {
    ...
    Position new_pos = get_position_from_direction(pacman->pos, map->dims, pacman->dir);    // Get new position
...
void draw_ghost(Ghost *_ghost) {
    ...
    Position new_pos = _ghost->next_position(_ghost, &map->dims);
